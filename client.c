...
void tap_To_net(int tap_fd, int net_fd, const struct sockaddr_in *remote)
{
    ...
    nwrite = sendto(net_fd, buffer, nread,0, (const struct sockaddr *)remote, sizeof(*remote));
...
int main(int argc, char *argv[])
{
    ...
    if (cliserv == CLIENT)
    {
        ...
        if (sendto(sock_fd, (const char *)hello, strlen(hello), 0, (const struct sockaddr *)&remote, sizeof(remote)) < 0)
        {
            perror("connect()");
            exit(1);
        }
